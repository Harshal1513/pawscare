...
const NAV = [
  ...
  { href: '/admin/appointments',   icon: '📅', label: 'Today\'s Appointments' },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-56 flex-col py-4 px-3 transition-transform
        ...
        <div className="flex items-center gap-2 px-2 mb-6 md:mb-8">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base" style={{ background: '#F59E0B' }}>🐾</div>
        ...
        <nav className="flex-1 space-y-1">
          {NAV.map(n => (
            <Link key={n.href} href={n.href}
              className={`flex items-center gap-2 text-[13.5px] font-semibold px-3 py-2.5 rounded-xl transition-all
                ${path === n.href
      ...
      <div className="flex-1 md:ml-56">
        ...
        <div className="bg-white border-b border-gray-100 px-4 md:px-6 py-3 md:py-4 flex items-center gap-3 md:gap-4 sticky top-0 z-30">
          ...
          <span className="font-extrabold text-sm md:text-base text-gray-900">
            ...
            <span className="hidden sm:inline">{NAV.find(n => n.href === path)?.label || 'Admin'}</span>
          </span>
